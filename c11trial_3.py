...
def greedy_best_first_search_recommend_with_diabetes_bmi_age_and_index(data, initial_recipe, max_depth, has_diabetes,visited):
    queue = deque([(initial_recipe, 0)])  # Queue of (recipe, depth)
    # visited = set()

    best_recipe = initial_recipe
    best_healthiness = evaluate_healthiness_with_diabetes_bmi_age(initial_recipe, has_diabetes)
    ...
    while queue:
        ...
        for _ in range(5):  # Consider 5 random possibilities
            ...
            if random_recipe['title'] not in visited:
                ...
                healthiness = evaluate_healthiness_with_diabetes_bmi_age(random_recipe, has_diabetes)
